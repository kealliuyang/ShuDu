...
function GameWin:ctor( param )
    ...
    self:addCsb( "csbmajiang/GameWin.csb" )
...
function GameWin:onEnter()
    ...
    audio.playSound("mjmp3/ef_win.mp3", false)
    -- 播放动画
    self:enterAction()
end

function GameWin:enterAction()
    ...
    local call_back = cc.CallFunc:create( function()
        ...
        -- 光的动画
        local rotate_to = cc.RotateBy:create(2, 360)
        local rep = cc.RepeatForever:create( rotate_to )
        self.Bg1:runAction( rep )
        -- 显示被数数据
        self:showTextAction()
        -- 显示增加的积分动画
        self:addScoreAction()
...
function GameWin:showTextAction()
    for i,v in ipairs( self._texts ) do
        local actions = {}
...
function GameWin:addScoreAction()
...
function GameWin:again()
    removeUIFromScene( UIDefine.MAJIANG_KEY.Win_UI )
