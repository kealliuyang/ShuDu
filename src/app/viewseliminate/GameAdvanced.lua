...
-- 界面数据
function GameAdvanced:loadDataUI( contineueData )
    -- 初始化积分
    self._totalScore = 0
    self.TextScore:setString( self._totalScore )
    ...
    self.TextBest:setPositionX(self.TextBest:getPositionX() + 25)
...
function GameAdvanced:saveContinueData()
    ...
    G_GetModel("Model_Eliminate"):saveContinueAdvancedData( put_block,create_block,score,refresh_times,self._countDownTime )
end
...
-- 增加倒计时的逻辑
function GameAdvanced:loadCountDownLogic( contineueData )
